using System;

// [AUTO-GENERATED] by BitterECS AutoPathConstantsGenerator
// Defines paths for Prefabs (Providers/MonoBehaviours) and ScriptableObjects

public static class UiPrefabsPaths
{
    /// <summary> Component: UIDistanceTraveledElement </summary>
    public const string UIDISTANCE_TRAVELED_ELEMENT = "UI/Prefabs/UIDistanceTraveledElement";
    /// <summary> Component: UIHandElement </summary>
    public const string UIHAND_ELEMENT = "UI/Prefabs/UIHandElement";
    /// <summary> Component: UIHealthElement </summary>
    public const string UIHEALTH_ELEMENT = "UI/Prefabs/UIHealthElement";
    /// <summary> Component: UISelectorElement </summary>
    public const string UISELECTOR = "UI/Prefabs/UISelector";
    /// <summary> Component: UIStackHandElement </summary>
    public const string UISTACK_HAND_ELEMENT = "UI/Prefabs/UIStackHandElement";
    public static readonly string[] AllPaths = new string[]
    {
        UIDISTANCE_TRAVELED_ELEMENT,
        UIHAND_ELEMENT,
        UIHEALTH_ELEMENT,
        UISELECTOR,
        UISTACK_HAND_ELEMENT,
    };
    public const int COUNT = 5;
}
